...
const CategoryDetail: React.FC = () => {
  ...
  const getIcon = () => {
    switch (categoryId) {
      case 'hate-speech':
        return <MessageSquareWarning className="h-8 w-8 text-navy" />;
      case 'disinformation':
        return <FileSearch className="h-8 w-8 text-navy" />;
      case 'digital-fraud':
        return <ShieldAlert className="h-8 w-8 text-navy" />;
      default:
        return <AlertTriangle className="h-8 w-8 text-navy" />;
    }
  };
  ...
  return (
    <div className="bg-white font-inter min-h-screen py-10">
      <div className="mx-auto max-w-[1200px] px-6">
        ...
        <div className="rounded-3xl border border-slate-200 bg-gradient-to-r from-navy/60 via-white to-white p-8 md:p-10 shadow-sm">
          <div className="flex items-center gap-4">
            <span className="flex h-16 w-16 items-center justify-center rounded-2xl bg-white shadow-md border border-slate-200">
              {getIcon()}
            </span>
            <div>
        ...
        <div className="mt-8 rounded-3xl border-2 border-orange-200 bg-gradient-to-r from-white via-orange-50/40 to-white p-6 md:p-8 shadow-sm">
          ...
          <div className="space-y-3">
            <textarea
              ...
              className="w-full rounded-2xl border border-slate-300 p-4 text-xs text-slate-900 focus:border-navy focus:outline-none bg-white shadow-inner"
            />
            <button
              onClick={() => handleRunAiCheck(checkInput)}
              className="px-6 py-3 rounded-xl bg-navy hover:bg-navyCard text-white font-extrabold text-xs shadow-md shadow-navy/20 hover:scale-[1.02] transition-all cursor-pointer"
            >
        ...
        <div className="mt-10 grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-3 mb-4 text-navy font-bold text-lg">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-50 text-navy">
              ...
              <div className="rounded-xl bg-slate-50 p-3 text-xs text-navy font-medium border border-slate-200/60">
                {detail.penalty}
              </div>
            </div>
          </div>
        ...
        <div className="mt-10 rounded-2xl border border-slate-200 bg-white p-8">
          ...
          <div className="space-y-4">
            {detail.examples.map((example, idx) => (
              <div key={idx} className="flex items-start gap-4 rounded-xl border border-slate-100 bg-slate-50/60 p-4">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-navy text-white font-bold text-xs">
                  {idx + 1}
                </span>
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <Link
              to="/contacts"
              className="inline-flex items-center gap-2 rounded-xl bg-navy px-5 py-2.5 text-xs font-bold text-white transition-all hover:bg-navyCard shadow-sm"
            >
