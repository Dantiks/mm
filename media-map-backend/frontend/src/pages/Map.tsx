...
const MapPage: React.FC = () => {
  ...
  const [activeModalMarker, setActiveModalMarker] = useState<MapMarker | null>(null);
  ...
  const filteredMarkers = markers.filter(m => {
    const matchesCategory = selectedCategory 
      ? (m.violationTypeId || m.violationType?.id) === selectedCategory 
      : true;
    
    const query = searchQuery.toLowerCase().trim();
    const matchesSearch = query === '' || 
      (m.authorComment && m.authorComment.toLowerCase().includes(query)) ||
      (m.moderatorComment && m.moderatorComment.toLowerCase().includes(query)) ||
      (m.authorCity && m.authorCity.toLowerCase().includes(query)) ||
      (m.authorRegion && m.authorRegion.toLowerCase().includes(query));
  ...
  return (
    <div className="bg-slate-50 min-h-screen font-inter">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12 -mt-8 relative z-10 pb-20">
        ...
        <div className="bg-white rounded-3xl shadow-xl p-4 border border-slate-200/80 mb-6 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
          
          {/* Categories Buttons */}
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={() => setSelectedCategory(null)}
              className={`flex items-center gap-2 px-4 py-2 rounded-2xl transition-all font-extrabold text-xs cursor-pointer ${
            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-2xl transition-all font-bold text-xs cursor-pointer ${
                    isSelected
                ...
                return (
                  ...
                  >
                    <Popup className="custom-leaflet-popup">
                      <div className="p-2 w-80 space-y-3 font-inter">
                        ...
                        {marker.image && (
                          <div
                            onClick={() => setActiveModalMarker(marker)}
                            className="w-full h-44 rounded-xl overflow-hidden bg-slate-100 border border-slate-200 cursor-pointer group relative"
                          >
                            <img
                              src={marker.image}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs">
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-6 relative max-h-[90vh] overflow-y-auto font-inter border border-slate-200">
            <button
              ...
              className="absolute right-5 top-5 p-2 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 transition-all cursor-pointer"
            >
              <X className="h-5 w-5" />
            </button>
            ...
            <h3 className="text-xl font-black text-navy mb-2">
              Детали зафиксированного нарушения #{activeModalMarker.id}
            </h3>
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 mb-6 pb-4 border-b border-slate-100">
              ...
              {activeModalMarker.createdAt && (
                <div className="flex items-center gap-1 font-mono">
                  ...
                  {activeModalMarker.createdAt.split('T')[0] || activeModalMarker.createdAt}
                </div>
              )}
            </div>

            {/* Screenshot full size */}
            {activeModalMarker.image && (
              <div className="w-full max-h-96 rounded-2xl overflow-hidden bg-slate-100 border border-slate-200 mb-6">
                <img
                  src={activeModalMarker.image}
                  alt="Полный скриншот нарушения"
